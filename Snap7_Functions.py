...
def get_data_array_from_plc_db(db_number, client, setup_file_step7): 
    try:
        ...
        data_index = get_plc_from_file(setup_file_step7).get('dbinsert data index') # get start index of data in dbinsert 
        data = client.db_read(db_number, data_index, setup_file_get_number_of_data_columns(setup_file_step7)*4) # get data bytearray from plc, assuming all data is 4 bytes long
...
def monitor_and_get_data_on_trigger_snap7(client, setup_file_step7):
    ...
    if (client != None): # check if client connected
        while trigger_value == 0: # wait for trigger ready from plc
            try:
                print(".")
                trigger_value = get_data_from_plc_db(db_number, client, logging_trigger_index) # get trigger state from plc
                if trigger_value == 1: # check if trigger ready from plc       
                    data_array = get_data_array_from_plc_db(db_number, client, setup_file_step7) # get dbinsert data 
                    client.db_write(db_number, logging_trigger_index, bytearray_to_write) # update logging trigger to signal python script done writing 
                ...
                while trigger_value == 0: # start loop to check for trigger cont    
                    try:
                        ...
                        if trigger_value == 1: # check for plc requesting data
                            ...
                            client.db_write(db_number, plc.get('dbresult time_sec index'), bytearray_to_time_sec) # write timespan to plc 
                            if bytearray_to_data != bytearray(b''): # check if bytearray is empty
                                client.db_write(db_number, plc.get('dbresult data index'), bytearray_to_data) # write data to plc if bytearray not empty
                            client.db_write(db_number, logging_trigger_index, bytearray_to_trigger) # update logging triger to signal python script done writing             
